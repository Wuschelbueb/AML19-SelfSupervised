...
class Algorithm():
    ...
    def set_log_file_handler(self):
        ...
        if (not os.path.isdir(log_dir)):
            os.makedirs(log_dir)
        # apparently windows can't handle : and you have to replace them with something else
    ...
    def init_all_networks(self):
        ...
        for key, val in networks_defs.items():
            ...
            net_opt = val['opt']
            self.optim_params[key] = val['optim_params'] if (
                'optim_params' in val) else None
            pretrained_path = val['pretrained'] if (
                'pretrained' in val) else None
    ...
    def init_optimizer(self, net, optim_opts, key):
        ...
        if optim_type == 'adam':
            ...
            optimizer = torch.optim.SGD(parameters, lr=learning_rate,
                                        ...
                                        nesterov=optim_opts['nesterov'] if (
                                            'nesterov' in optim_opts) else False,
                                        weight_decay=optim_opts['weight_decay'])
        else:
            raise ValueError(
                'Not supported or recognized optim_type', optim_type)
    ...
    def run_train_epoch(self, data_loader, epoch):
        ...
        # Can't pickle local object 'dataloader.get-iterator.locals... not supported on windows
    ...
    def adjust_learning_rates(self, epoch):
        ...
        for key, oparams in optim_params_filtered.items():
            LUT = oparams['LUT_lr']
            lr = next((lr for (max_epoch, lr)
                       in LUT if max_epoch > epoch), LUT[-1][1])
    ...
    def keep_record_of_best_model(self, eval_stats, current_epoch):
        if self.keep_best_model_metric_name is not None:
            ...
            if (metric_name not in eval_stats):
                raise ValueError(
                    'The provided metric {0} for keeping the best model is not computed by the evaluation routine.'.format(metric_name))
