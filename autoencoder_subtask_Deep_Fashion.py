...
def autoencoder_subtask_deep_fashion():
    ...
    encoder_trained, train_losses = train_autoencoder_deep_fashion()
    plot_n_curves(train_losses, "Train losses", "Loss training AET DeepFashion", axis2="Loss")

    # transfer learning with DeepFashion
    encoder_ft, train_losses_ft, val_losses_ft, train_acc_ft, val_acc_ft = transfer_learning_autoencoder_deep_fashion(encoder_trained)
    ...
    plot_summary([train_acc_ft, val_acc_ft], average_test_accuracy, ["train accuracy", "val accuracy", "test accuracy"], "Accuracy autoencoder DeepFashion", axis2="Accuracy")
    plot_summary([train_losses_ft, val_losses_ft], average_test_loss, ["train loss", "val loss", "test average loss"], "Loss autoencoder DeepFashion", axis2="Loss")
