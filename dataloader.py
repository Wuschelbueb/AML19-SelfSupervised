...
class GenericDataset(data.Dataset):
    def __init__(self,
        ...
        if self.dataset_name == 'fashionmnist':
            ...
            if self.random_sized_crop:
                raise ValueError(
                    'The random size crop option is not supported for the fashionmnist dataset'
                )

            transform = []
            if (split != 'test'):
                transform.append(transforms.RandomCrop(28, padding=4))
                transform.append(transforms.RandomHorizontalFlip())
            transform.append(lambda x: np.asarray(x))
            self.transform = transforms.Compose(transform)
            self.data = datasets.__dict__['FashionMNIST'](
                _FASHION_DATASET_DIR,
    ...
    def _keep_first_k_examples_per_category(self, num_imgs_per_cat):
        ...
        if self.dataset_name == 'fashionmnist':
            ...
            raise ValueError(
                'Keeping k examples per category has not been implemented for the {0}'
            ...
            raise ValueError('Not recognized dataset {0}'.format(
                self.dataset_name))
